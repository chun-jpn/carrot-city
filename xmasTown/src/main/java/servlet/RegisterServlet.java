...
public class RegisterServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (userName.isEmpty() || address.isEmpty() || tel.isEmpty() || mail.isEmpty()
        || password.isEmpty()) {
      // エラーメッセージをセットしてフォームを再表示
      request.setAttribute("error", "名前、住所、電話番号、メールアドレス、パスワードは必須項目です。");
      request.getRequestDispatcher("WEB-INF/jsp/register.jsp").forward(request, response);
      System.out.println("名前、住所、電話番号、メールアドレス、パスワードは必須項目です。");
      ...
      if (registerDAO.isMailAlreadyRegistered(mail)) {
        ...
        request.setAttribute("error", "既に同じメールアドレスが登録されています。別のメールアドレスを入力してください。");
        request.getRequestDispatcher("WEB-INF/jsp/register.jsp").forward(request, response);
        System.out.println("既に同じメールアドレスが登録されています。別のメールアドレスを入力してください。");
      } else {
        // 重複がない場合は新しいユーザを登録
